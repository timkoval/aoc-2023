...
impl From<char> for Card {
    fn from(c: char) -> Self {
        match c {
            ...
            '3' => Card::Three,
            '2' => Card::Two,
            // making J the weakest card for the second part
            'J' => Card::J,
...
pub fn solve() -> u64 {
    let input = include_str!("../inputs/card.txt");
    input
        .split('\n')
        .filter_map(|line| {
            if line.is_empty() {
            ...
            Some((
                Hand::new(cards.chars().collect::<Vec<_>>()),
                bid.parse::<u64>().unwrap(),
            ))
        })
        .sorted_by(|a, b| a.0.partial_cmp(&b.0).unwrap())
        .enumerate()
        .inspect(|(rank, (hand, bid))| println!("{}: {:?} {}", rank, hand, bid))
        .map(|(i, (_, num))| (i + 1) as u64 * num)
        .sum()
}
